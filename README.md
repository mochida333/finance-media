...
#### すでに存在するコンテンツがある場合（再実行したい）

microCMS 側に同じ `contentId` が既にある場合、標準だと `PUT` が 400 で失敗します。
再実行・差分反映をしたい場合は `--upsert`（既存なら `PATCH`）か `--skip-existing`（既存はスキップ）を使ってください。

```sh
# 既存なら PATCH で更新
node scripts/wp-import.mjs --xml "/path/to/export.xml" --apply --upsert --create-categories --create-tags

# 既存は触らずスキップ
node scripts/wp-import.mjs --xml "/path/to/export.xml" --apply --skip-existing --create-categories --create-tags
```

#### 記事のスラッグを microCMS のコンテンツIDにしたい

WordPress の `wp:post_name`（slug）を microCMS 側の `contentId` に使いたい場合は、`--use-slug-as-content-id` を付けます。

```sh
node scripts/wp-import.mjs --xml "/path/to/export.xml" --apply --use-slug-as-content-id --create-categories --create-tags
```

> slug が空/日本語のみ等で `contentId` にできない場合や、重複する場合は、スクリプト側でサニタイズ・ユニーク化し、必要に応じて従来方式（`wpPostId`）へフォールバックします。
> また、microCMS の `contentId` 制約に合わせて **英数字 + `-` + `_` のみ**、**最大50文字**に収まるように自動調整します。
> WordPress 側の slug が `%e3%81%82...` のように percent-encoded で入っている場合は、衝突しないようにハッシュを付与したIDに変換します（例: `e38182...-a1b2c3d4e5`）。
